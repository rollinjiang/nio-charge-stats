...
- **更新频率**：每天 2 次自动抓取（北京时间 08:30 / 20:30，GitHub Actions 云端定时，无需本机开机）
- **在线看板**：https://nio-charge-stats-v2.app.workbuddy.host/
- **分省看板**：https://nio-charge-stats-v2.app.workbuddy.host/province.html
...
| 页面 | 地址 | 内容 |
|---|---|---|
| 全国看板 `index.html` | `/` | 6 张 KPI 卡片 + 「每月新建换电站数量」柱状图 + 「每月新建充电站数量」柱状图 |
...
### 本期新增口径
- **本月新增换电站** = 当前累计换电站 − 项目历史起点 **2026-09-07** 的累计换电站。
- **本年新增换电站** = 当前累计换电站 − **3,676**（蔚来官方公布 2025-12-31 换电站总数，固定基准值）。
- **本月新增换电站最多的省份**：比较各省当前累计 − 2026-09-07 该省累计，取增量最大者。

### 每月新建口径（2026-09-24 新增，持续保存）
首页两张柱状图分别展示 **每月新建换电站数量** 与 **每月新建充电站数量**：

- **公式**：某月新建数 = 该月最后一条记录累计 − 基线累计
  - **首月（2026-09）**：基线 = 当月首条记录（9/7）：换电站 4,071 / 充电站 5,252
  - **后续月份**：基线 = 上一个月最后一条记录累计（即上月月末值）
- **持久化**：结果写入 `data/monthly_new.json`；每次聚合会**重算 + 合并旧快照**，已完结月份数值锁定，即使历史被清理也不会丢失柱状图。
- **完结判定**：只要存在更晚且有数据的月份，该月即标记 `is_complete=true`（数值锁定）。
- **当前月状态**：数值随每日抓取持续波动，跨月后自动锁定并计入下月基线。
- 前端由 `buildMonthlyBarOption()` 统一渲染柱状图（柱顶显示数值，tooltip 显示区间/天数/完结状态）。
