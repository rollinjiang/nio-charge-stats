...
| 页面 | 地址 | 内容 |
|---|---|---|
| 全国看板 `index.html` | `/` | 全国充换电站/换电站/高速换电站四维趋势 + 本月/本年新增换电站 |
| 分省统计 `province.html` | `/province.html` | 全国31省换电站总数 · 排序表 + 趋势 + 本月新增最多省份 + 深圳总数 |
...
### 本期新增口径（2026-09-07 新增）
- **本月新增换电站** = 当前累计换电站 − 本月 1 日之前最后一条记录累计；首次统计（无基线）显示为当前累计。
- **本年新增换电站** = 当前累计换电站 − 本年 1 月 1 日之前最后一条记录累计；首次统计（无基线）显示为当前累计。
- **本月新增换电站最多的省份**：比较各省当前累计 − 本月 1 日之前最后一条记录累计，取增量最大者。
- **深圳市换电站总数**：用地级市代码 `440300` 单独查询接口（与省级同维度 `region_code`），当前 94 座。
